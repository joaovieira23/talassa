...
function ProductsScreen(props) {
  ...
  return <div className="content content-margined">
    ...
    <div className="product-header">
      <h3>Products</h3>
      <button className="button primary" style={{ width: "350px" }} onClick={() => openModal({})}>Create Product</button>
    </div>
    {modalVisible &&
      <div className="form">
        <form onSubmit={submitHandler} >
          <ul className="form-container">
            ...
            <li>
              <button type="submit" style={{ width: "322px" }} className="button primary">{id ? "Update" : "Create"}</button>
            </li>
            <li>
              <button type="button" style={{ width: "322px" }} onClick={() => setModalVisible(false)} className="button secondary">Back</button>
            </li>
          </ul>
        </form>
      </div>
      ...
      <table className="table">
        ...
        <tbody>
          {products.map(product => (<tr key={product._id}>
            ...
            <td>{product.brand}</td>
            <td>
              <button className="button" style={{ width: "200px", height: "44px", marginBottom: "15px" }} onClick={() => openModal(product)} >Edit</button>
              {' '}
              <button className="button" style={{ width: "200px", height: "44px", marginBottom: "15px" }} onClick={() => deleteHandler(product)} >Delete</button>
            </td>
          </tr>))}
        </tbody>
      </table>
